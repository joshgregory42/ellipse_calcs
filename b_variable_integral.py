...
b_values = np.linspace(int(lower), int(upper), int(num))
...
print("\nSubstituting b-values into integrands...")

# This loop puts the array of a-values into the bounds and the integrand
for k in range(0, len(b_values)):
    # Substituting in the a-values from a-values array into the bounds

    bounds_1_up.append(-1 * (b_values[k] * sympy.sqrt(1 / (b_values[k] ** 4 + 1))))
    bounds_1_low.append(b_values[k] * sympy.sqrt(1 / (b_values[k] ** 4 + 1)))

    function_1_integrand.append(integrand[0].subs(b, b_values[k]))

function_1_area = []

print("\nIntegrating over all " + str(num) + " b-values...")

for q in range(0, len(b_values)):
...
workbook = xlsxwriter.Workbook('b_var.xlsx')
